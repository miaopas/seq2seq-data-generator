...
normal_path = 'dataset\\0\\'
inst_path = 'dataset\\4\\' # instability data
...
def w3_generator(length, train_test_ratio, overlap_ratio = 0, sliding_window = False):
    ...
    train_normal = read_files(normal_path, trainlist_normal, length, overlap_ratio, sliding_window)
    train_normal_output = np.zeros(train_normal.shape[0])
    # build instable train dataset
    train_inst = read_files(inst_path, trainlist_inst, length, overlap_ratio, sliding_window)
    ...
    test_normal = read_files(normal_path, testlist_normal, length, overlap_ratio, sliding_window)
    test_normal_output = np.zeros(test_normal.shape[0])
    # build instable test dataset
    test_inst = read_files(inst_path, testlist_inst, length, overlap_ratio, sliding_window)
...
if __name__ == "__main__":
    train_input, train_output, test_input, test_output = w3_generator(900, 0.7, overlap_ratio = 0)
